...
* **Key Design Pattern:** **Web Worker Offloading**.  
  ...
  * *Rationale:* Ensures the UI never freezes, even during heavy data processing.
...
* **Key Mechanism:** **Server-Side HMAC Signing**.  
  ...
  * It injects an X-Signature header or body field using a secret stored ONLY in the Nginx container (HMAC\_SECRET).  
  * *Rationale:* Prevents "Inspect Element" cheating. Students never possess the secret key required to forge a valid payload.
...
* **Concurrency Strategy:** Uses **Redis** to buffer incoming webhooks. Worker nodes process the queue asynchronously, preventing bottlenecks during the "class-start surge."
...
* **Connection Pooling:** **PgBouncer** in transaction mode.  
  ...
  * *Rationale:* Prevents PostgreSQL process exhaustion on limited hardware (8GB RAM).

## **3\. Infrastructure & Deployment**

* **Orchestration:** Docker Compose.  
* **Network Isolation:**  
  * frontend\_net: Public-facing. Connects Nginx and n8n-main.  
  * backend\_net: Internal only. Connects n8n, Redis, PgBouncer, and Postgres.  
* **Self-Healing:**  
...
* **Secret Management:** Secrets (POSTGRES\_PASSWORD, HMAC\_SECRET) are injected via .env file at runtime and never committed to version control.  
...
4. **Sign:** Nginx signs payload using HMAC\_SECRET.  
